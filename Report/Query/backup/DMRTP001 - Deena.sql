...
CASE WHEN B.BA_STATUS = 'Active' THEN
        ...
        AND M.MOBILE_STATUS_DTM = (SELECT MAX(M.MOBILE_STATUS_DTM)
...
TRIM(B.BILL_ADDRESS_LINE_1 ||' '|| B.BILL_ADDRESS_LINE_2 ||' '|| B.BILL_ADDRESS_LINE_3 ||' '|| B.BILL_ADDRESS_LINE_4 ||' '|| B.BILL_ADDRESS_LINE_5) AS BILL_ADDRESS,

B.BILL_ZIPCODE,

(SELECT S1.CONTACT_MOBILE_PHONE FROM SFF_ACCOUNT S1 WHERE S1.ROW_ID = B.SFF_ACCOUNT_ID) AS BA_CONTACT_TEL, 

TRIM(C.VAT_ADDRESS_LINE_1 ||' '|| C.VAT_ADDRESS_LINE_2 ||' '|| C.VAT_ADDRESS_LINE_3 ||' '|| C.VAT_ADDRESS_LINE_4 ||' '|| C.VAT_ADDRESS_LINE_5) AS HOME_ADDRESS,

C.VAT_ZIPCODE AS HOME_ZIPCODE

(SELECT S1.CONTACT_MOBILE_PHONE FROM SFF_ACCOUNT S1 WHERE S1.ROW_ID = C.SFF_ACCOUNT_ID) AS CA_CONTACT_TEL
...
I.BILL_START_DATE ||' - '|| I.BILL_END_DATE AS BILL_CYCLE,
...
FROM PM_INVOICE I, CL_HIS_BA_INVOICE_BALANCE IB 
